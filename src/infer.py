...
def load_model(arch: str, num_classes: int, weights_path: Path) -> torch.nn.Module:
    if arch == "baseline":
        ...
        model = create_transfer_model(arch, num_classes=num_classes, pretrained=False)
...
def main():
    parser = argparse.ArgumentParser(description="Infer cat vs dog from image")
    parser.add_argument("--image", type=str, required=True)
    parser.add_argument("--arch", type=str, default="resnet18")
    parser.add_argument("--label-map", type=str, default="checkpoints/label_map.json")
